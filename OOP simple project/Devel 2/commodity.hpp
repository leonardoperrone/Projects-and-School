...
// The structure below is defined here for use by derived classes so that
// a single definition can be used by any derived class that needs it.

struct CommodityDependency
{
   CommodityDependency(Commodity* c, int n, int r);
   Commodity* commodity;
   int number;
   int ratio;
};
